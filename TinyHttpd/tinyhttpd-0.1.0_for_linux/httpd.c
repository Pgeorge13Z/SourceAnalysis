...
//函数说明：检查参数c是否为空格字符，用来解析HTTP协议用到
...
void *accept_request(void* client); //处理连接、子进程
void bad_request(int);//无效请求 400错误
void cat(int, FILE *); //处理文件，读取文件内容发送到客户端
void cannot_execute(int);//500错误，服务器无法完成请求
...
void not_found(int); //404,请求内容不存在
void serve_file(int, const char *);  //处理文件请求
int startup(u_short *); //初始化服务器
void unimplemented(int); //501 服务器不支持请求的功能，仅实现了get post方法，其他方法错误处理函数
...
//设置http socket，IPV4,TCP
 httpd = socket(PF_INET, SOCK_STREAM, 0);
 if (httpd == -1) //初始化socket失败
  error_die("socket");
 memset(&name, 0, sizeof(name)); //初始化name,用name长度的0填充name
 name.sin_family = AF_INET; //地址类型ipv4（具体结构件info.md中写的结构体）
 name.sin_port = htons(*port); //端口转换为传递过来的端口，htons：将整型变量从主机字节顺序转变成网络字节顺序，大端存储
 name.sin_addr.s_addr = htonl(INADDR_ANY); //本机任一可用的ip地址
 //手动更改地址形式的方法：inet_pton(AF_INET,"192.168.1.1",&name.sin_addr.s_addr);

 //绑定端口
 if (bind(httpd, (struct sockaddr *)&name, sizeof(name)) < 0)
  error_die("bind");
 if (*port == 0)  //*动态分配一个端口 如果传递的端口号是0，则随机选取可用的端口
 {
  socklen_t  namelen = sizeof(name);
  if (getsockname(httpd, (struct sockaddr *)&name, &namelen) == -1)
   error_die("getsockname");
  *port = ntohs(name.sin_port);  //与上面的htons对应，将网络字节序转化为主机字节序
 }
 //监听连接
 if (listen(httpd, 5) < 0)
  error_die("listen");
...
int main(void)
{
 ...
 struct sockaddr_in client_name;  //定义sockaddr_in 型结构体，accept阶段用来获取客户端地址信息
 socklen_t client_name_len = sizeof(client_name);  //客户端地址信息的长度
 pthread_t newthread;  //定义线程id

//启动server socket，初始化服务器
